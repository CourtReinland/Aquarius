...
export function BlueAssistantAvatar({
  ...
  const mouthStyle = buildMouthStyle(mouthFrame, size);

  return (
    <View style={[styles.shell, { width: size + 36 }]}>
      <Animated.View style={[styles.avatarFloat, avatarStyle]}>
...
function buildMouthStyle(frame: BlueMouthFrame, size: number) {
  const width = size * (0.05 + frame.width * 0.055);
  const height = size * (0.012 + frame.openness * 0.065);
